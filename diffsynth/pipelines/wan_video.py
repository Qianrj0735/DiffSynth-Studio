...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        # Encode prompts uncomment 2 lines below for customized prompts inputs
        ...
        for progress_id, timestep in tqdm(
            enumerate(progress_bar_cmd(self.scheduler.timesteps))
        ):
            timestep = timestep.unsqueeze(0).to(
                dtype=self.torch_dtype, device=latents.device
            )
...
def model_fn_wan_video(
    ...
    latent_indices: torch.Tensor = None,
